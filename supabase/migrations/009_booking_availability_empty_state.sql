-- A missing creator_profiles row means availability is not configured, not that
-- the creator is unavailable. Explicit busy/unavailable states remain blocked.
...
begin
  ...
  if not exists (
    ...
    where p.id = p_creator_id
      and p.role = 'creator'
      and coalesce(cp.availability, 'available') = 'available'
  ) then
  ...
  values (p_creator_id, 'new_request', 'New service request', 'You have received a new service request.', request_row.id);
